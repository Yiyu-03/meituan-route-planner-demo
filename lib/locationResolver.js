...
/** Test helper: clear the anchor center cache between cases. */
export function __clearAnchorCache() {
  anchorCache.clear();
}
...
/**
 * Resolve an anchor (区域名 like 静安, or 具体地点 like 新世界城) to a center {lng,lat}.
 * Geocode first (city + anchor), then place/text first result. Cached. null on failure —
 * the caller must fall back to district center / densest-cluster, never fabricate coords.
 */
export async function resolveAnchor(anchorText, city = '', deps = {}) {
  ...
  try {
    ...
    // 2) fallback: place/text first result's location
    if (!center) {
